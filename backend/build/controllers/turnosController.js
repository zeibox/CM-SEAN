...
class TurnosController {
    async list(req, res) {
        const dato = await database_1.default.query('SELECT * FROM v_turnos');
        res.json(dato);
    }
    async getOne(req, res) {
        const { id } = req.params;
        const dato = await database_1.default.query('SELECT * FROM v_turnos WHERE id_turno = ?', [id]);
        if (dato.length > 0) {
    ...
    async delete(req, res) {
        const { id } = req.params;
        await database_1.default.query('DELETE FROM v_turnos WHERE id_turno = ?', [id]);
