...
class MedicosController {
    async list(req, res) {
        const dato = await database_1.default.query('SELECT * FROM medicos');
        res.json(dato);
    }
    async getOne(req, res) {
        const { id } = req.params;
        const dato = await database_1.default.query('SELECT * FROM medicos WHERE id_medico = ?', [id]);
        if (dato.length > 0) {
    ...
    async delete(req, res) {
        const { id } = req.params;
        await database_1.default.query('DELETE FROM medicos WHERE id_medico = ?', [id]);
