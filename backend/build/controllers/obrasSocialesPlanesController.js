...
class ObrasSocialesPlanesController {
    async list(req, res) {
        const dato = await database_1.default.query('SELECT * FROM obras_sociales_planes');
        res.json(dato);
    }
    async getOne(req, res) {
        const { id } = req.params;
        const dato = await database_1.default.query('SELECT * FROM obras_sociales_planes WHERE id_osp = ?', [id]);
        if (dato.length > 0) {
    ...
    async delete(req, res) {
        const { id } = req.params;
        await database_1.default.query('DELETE FROM obras_sociales_planes WHERE id_osp = ?', [id]);
