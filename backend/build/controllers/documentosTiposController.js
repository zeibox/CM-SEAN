...
class DocumentosTiposController {
    async list(req, res) {
        const dato = await database_1.default.query('SELECT * FROM documentos_tipo');
        res.json(dato);
    }
    async getOne(req, res) {
        const { id } = req.params;
        const dato = await database_1.default.query('SELECT * FROM documentos_tipo WHERE id_documento_tipo = ?', [id]);
        if (dato.length > 0) {
    ...
    async delete(req, res) {
        const { id } = req.params;
        await database_1.default.query('DELETE FROM documentos_tipo WHERE id_documento_tipo = ?', [id]);
