...
class JerarquiasController {
    ...
    public async list(req: Request, res: Response): Promise<void> {
        const dato = await bd.query('SELECT * FROM jerarquias');
        res.json(dato);
    }

    public async getOne(req: Request, res: Response): Promise<any> {
        const { id } = req.params;
        const dato = await bd.query('SELECT * FROM jerarquias WHERE id_jerarquia = ?', [id]);
        if (dato.length > 0) {
    ...
    public async create(req: Request, res: Response): Promise<void> {
        const result = await bd.query('INSERT INTO jerarquias set ?', [req.body]);
    ...
    public async update(req: Request, res: Response): Promise<void> {
        ...
        await bd.query('UPDATE jerarquias set ? WHERE id_jerarquia = ?', [req.body, id]);
        res.json({ message: "Jerarquía actualizada" });
    }

    public async delete(req: Request, res: Response): Promise<void> {
        const { id } = req.params;
        await bd.query('DELETE FROM jerarquias WHERE id_jerarquia = ?', [id]);
