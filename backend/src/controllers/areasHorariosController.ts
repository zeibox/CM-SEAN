...
class AreasHorariosController {
    ...
    public async list(req: Request, res: Response): Promise<void> {
        const dato = await bd.query('SELECT * FROM v_areas_horarios');
        res.json(dato);
    }

    public async getOne(req: Request, res: Response): Promise<any> {
        const { id } = req.params;
        const dato = await bd.query('SELECT * FROM v_areas_horarios WHERE id_area = ?', [id]);
        if (dato.length > 0) {
    ...
    public async delete(req: Request, res: Response): Promise<void> {
        const { id } = req.params;
        await bd.query('DELETE FROM v_areas_horarios WHERE id_area = ?', [id]);
